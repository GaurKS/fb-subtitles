...
const handler = async (event) => {
  try {
    ...
    if (postType == 'Facebook' && post.subtitle.length != 0) {
      ...
      const captionUrl = `https://graph.facebook.com/v19.0/458156056572266/captions?access_token=${accessToken}&default_locale=none&locales_to_delete=[]`
      for (const subtitle of subtitleGlobalFilePath) {
        console.log('subtitle path for current:', "@/" + subtitle)
        ...
        // use axios and pass the file in form data
        const filePath = path.resolve(subtitle);
        const formData = new FormData();
        console.log("filePath: ", filePath);
        console.log("Printitng form data: ", fs.createReadStream(filePath))

        formData.append('caption_file', fs.createReadStream(filePath));
        formData.append('method', 'POST');
        console.log("formData: ", formData);
        console.log("captionUrl: ", captionUrl);

        const urlData = await axios.post(captionUrl, formData);
